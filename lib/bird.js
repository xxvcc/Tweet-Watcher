...
function fetchTweets({ birdPath, username, count, authToken, ct0 }) {
  return new Promise((resolve) => {
    ...
    // bird 0.8.0 支持 AUTH_TOKEN / CT0。凭据不得进入 argv：同机其他 UID 通常能读取
    // /proc/<pid>/cmdline，而受系统 ptrace 边界保护的 environ 不向这些 UID 开放。
    const args = ['user-tweets', username, '--json', '-n', String(n), '--no-color'];
    const env = {};
    // 只继承 CLI 运行所需的通用环境，避免把 Web 服务的代理密钥等无关变量传给子进程。
    for (const key of ['PATH', 'HOME', 'LANG', 'LC_ALL', 'LC_CTYPE', 'TZ', 'TMPDIR', 'TMP', 'TEMP']) {
    ...
    execFile(birdPath, args, {
      env, timeout: 30000, killSignal: 'SIGKILL', maxBuffer: 8 * 1024 * 1024,
    }, (err, stdout, stderr) => {
      if (err) {
        if (err.code === 'ENOENT') {
