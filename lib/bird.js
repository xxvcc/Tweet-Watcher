...
function normalizeTweets(data, account) {
  ...
  for (const it of items) {
    ...
    out.push({
      id,
      text: firstNonEmpty(it.text, it.full_text, it.content),
      time: firstNonEmpty(it.created_at, it.createdAt, it.time, it.date, it.published_at),
      // 只接受字符串型 url；对象/数字会被 String() 变成 "[object Object]" 之类的垃圾链接
      url: (typeof it.url === 'string' && it.url) ? it.url : `https://x.com/${account}/status/${id}`,
